...
def signup(request): #How we handle signups and logins
	if request.method == 'POST': 								#If a person filled out the sign up form
		...
		#print(comment1)
		p = Profile(username=request.user, name=profile1['name'], gender=profile1['gender'], major=profile1['major'], age=profile1['age'], description=profile1['description'])
		p.save()
		qs = Profile.objects.all()
		return HttpResponseRedirect('/virtualstudybuddy/profile/'+str(p.id)+'/')
	else: 														#If a person just logged in
		qs = Profile.objects.all()
		if qs.filter(username=request.user).exists(): 				#If user is already signed up
			p = qs.filter(username=request.user)[0]
